<template>
	<div>
		...
		<base-button @click="modal = true" class="float-right d-inline-block" type="primary">Add expertise</base-button>
		...
		<section v-else>
			<table class="table table-hover">
				...
				<tbody v-else>
					<expertise-row v-for="expertise in expertises" 
                         :expertise="expertise" 
                         :key="expertise.id"/>
				</tbody>
			</table>
    ...
    <modal :show.sync="modal">
        <h6 slot="header" class="modal-title" id="modal-title-default">New expertise</h6>

        <v-select label="name" v-model="course" :options="courses"></v-select>

        <template slot="footer">
            <base-button type="primary" @click="submit">Create</base-button>
