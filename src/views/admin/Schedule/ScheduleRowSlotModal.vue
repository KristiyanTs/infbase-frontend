<template>
  <modal :show.sync="modal_prop" class="text-left" @close="closeModal">
    <h6 slot="header" class="modal-title" id="modal-title-default">Edit time slot</h6>
    <base-alert type="warning" v-if="errors">
      {{errors}}
    </base-alert>

    <vSelect v-if="tutors" v-model="tutor"
              label="first_name" :options="tutors" />
...
export default {
  ...
  computed: {
    dayOfWeek () {
      if (this.day_prop)
        return new Date(...this.day_prop).toLocaleDateString('en-uk', {'weekday': 'long'});
    }
  },
  watch: {
    modal_prop () {
